...
class MatrixNormalWishart():
    ...
    def __init__(self,mu_0,U_0=None,V_0=None,mask=None,X_mask=None,pad_X=False,fixed_precision=False):
        self.n = mu_0.shape[-2]
        self.p = mu_0.shape[-1]
        self.pad_X = pad_X
        ...
        if V_0 is None:
            ...
            temp = V_0.diagonal(dim1=-1,dim2=-2).mean(-1)
            V_0 = torch.cat((V_0,torch.zeros(V_0.shape[:-1]+(1,),requires_grad=False)),dim=-1)
            V_0 = torch.cat((V_0,torch.zeros(V_0.shape[:-2]+(1,)+V_0.shape[-1:],requires_grad=False)),dim=-2).detach().clone()
            V_0[...,-1,-1] = temp

        self.mask = mask
        self.X_mask = X_mask
        self.mu_0 = mu_0
        self.mu = torch.randn_like(mu_0,requires_grad=False)/np.sqrt(self.p)+mu_0
    ...
    def ss_update(self,SExx,SEyx,SEyy,N,lr=1.0,beta=None):
        ...
        if self.fixed_precision is False:
            ...
            self.invU.ss_update(SEyy,N,lr=lr,beta=None)
...
#            PJ11 = torch.tensor(0.0)
    ...
    def EXmMUTAXmMU(self,A):
        return self.V*(self.invU.ESigma()*A).sum(-1).sum(-1)
