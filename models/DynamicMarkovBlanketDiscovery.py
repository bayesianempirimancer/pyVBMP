...
class DynamicMarkovBlanketDiscovery(LinearDynamicalSystems):
    def __init__(self, obs_shape, role_dims, hidden_dims, control_dim = 0, regression_dim = 0, batch_shape=(),number_of_objects=1, unique_obs = False):
        ...
        if(number_of_objects>1):
            ...
            hidden_dim = sum(hidden_dims)
            role_dim = sum(role_dims)
...
#        self.latent_noise = latent_noise
        ...
        self.x0.mu = torch.zeros_like(self.x0.mu,requires_grad=False)
        
        self.A = MatrixNormalGamma(offset + (hidden_dim,hidden_dim+control_dim), batch_shape, mask = A_mask, pad_X=False, uniform_precision=False)
...
#        invSigma, invSigmamu, Residual = self.obs_model.Elog_like_X_given_pY((Delta(Y.unsqueeze(-unsdim)),R.unsqueeze(-unsdim))) 
    ...
    def update_assignments(self,y,r):
        ...
        self.SEzz, self.SEz0, self.NA, logZ = self.obs_model.update_states((px4r,r.unsqueeze(-unsdim),y.unsqueeze(-unsdim)))

    def update_obs_parms(self,y,r,lr=1.0):
        self.obs_model.update_markov_parms(self.SEzz, self.SEz0, lr)
        target_shape = r.shape[:-2]  
        unsdim = self.obs_model.event_dim + 2 
        px4r = MultivariateNormal_vector_format(mu = self.px.mu.expand(target_shape + (self.hidden_dim,1)),
                                                Sigma = self.px.Sigma.expand(target_shape + (self.hidden_dim,self.hidden_dim)),
    ...
    def update(self,y,u,r,iters=1,latent_iters = 1, lr=1.0, verbose=False):
        ...
        for i in range(iters):
            ...
            if verbose is True:
                print('Percent Change in ELBO = ',((ELBO-self.ELBO_last)/self.ELBO_last.abs())*100,  '   Iteration Time = ',(time.time()-t))
    ...
    def ELBO(self):
        ...
        ELBO_contrib_obs = (self.obs_model.transition.loggeomean()[mask_temp]*self.SEzz[mask_temp]).sum()
        ELBO_contrib_obs = ELBO_contrib_obs + (self.obs_model.initial.loggeomean()*self.SEz0).sum()
...
#### DMBD MASKS
    ...
    def one_object_mask(self,hidden_dims,role_dims,control_dim,obs_dim,regression_dim):
        ...
        Ab = torch.ones(hidden_dims[1],sum(hidden_dims[0:3]),requires_grad=False)
        ...
        if(len(hidden_dims)==4):
            ...
            Ag = torch.cat((torch.zeros(hidden_dims[3],sum(hidden_dims[:-1]),requires_grad=False),torch.ones(hidden_dims[3],hidden_dims[3])),dim=-1)
