...
class BayesNet():
    ...
    def __init__(self, n, p, hidden_dims, mixture_dims, batch_shape=(),pad_X=True):
        ...
        Input_Dist = dMixtureofLinearTransforms
        Layer_Dist = dMixtureofLinearTransforms
        Output_Dist = MultiNomialLogisticRegression
        self.layers = [Input_Dist(hidden_dims[0],p,mixture_dims[0],batch_shape=batch_shape,pad_X=True)]
        ...
        for i in range(1,self.num_layers):
            self.layers.append(Layer_Dist(hidden_dims[i],hidden_dims[i-1],mixture_dims[i],batch_shape=batch_shape,pad_X=True))
            ...
            #     pad_X=True))
        self.layers.append(Output_Dist(n,hidden_dims[-1],batch_shape = batch_shape,pad_X=True))
        self.MSE = []
        self.ELBO_save = []
...
#            Y_pred = self.layers[-1].forward(pX_forward[-1]).mean().squeeze(-1)
            ...
            pX_backward[-1] = self.layers[-1].backward(Y)[0]
            ...
            # FBI step -1 HERE
            if FBI is True:
                self.layers[-1].update(pX[-1],Y,lr=lr)
                pX_backward[-1] = self.layers[-1].backward(Y)[0]
                pX[-1] = MultivariateNormal_vector_format(invSigma = pX_forward[-1].EinvSigma() + pX_backward[-1].EinvSigma(), 
                        invSigmamu = pX_forward[-1].EinvSigmamu() + pX_backward[-1].EinvSigmamu())
...
#                self.layers[-1].update(pX[-1],Delta(Y.unsqueeze(-1)),lr=lr)
            ...
            if verbose:
                print('Percent Change in ELBO = ',(self.ELBO-self.ELBO_last)/self.ELBO_last.abs())
    ...
    def Elog_like(self,X,Y,qX):
        ...
        Res = Res + self.layers[-1].Elog_like_given_pX_pY(qX[-1],Y)
        for q in qX:
            Res = Res - q.Res()
        return Res        
    ...
    def predict(self,X):
        pX_forward = self.layers[0].predict(X)[0]
#        pX_forward = MultivariateNormal_vector_format(mu=mu, Sigma=Sigma)  # forward message into layer 1
