...
class ARHMM(HMM):
    def __init__(self,dim,n,p,batch_shape = (),pad_X=True,X_mask = None, mask=None, transition_mask=None):
        dist = MatrixNormalWishart(event_shape = (n,p), batch_shape = batch_shape + (dim,),pad_X=pad_X,X_mask=X_mask,mask=mask)
    ...
    def update_obs_parms(self,XY,lr,beta):
        self.obs_dist.raw_update(XY[0],XY[1],p=self.p,lr=lr,beta=beta)
...
class ARHMM_prXY(HMM):
    ...
    def update_obs_parms(self,XY,lr,beta=None):
        self.obs_dist.update(XY[0],XY[1],self.p,lr=lr,beta=beta)
...
class ARHMM_prXRY(HMM):   # Assumes that R and Y are observed
    def __init__(self,dim,n,p1,p2,batch_shape=(),mask=None,X_mask = None, transition_mask = None, pad_X=False):
        self.p1 = p1
        self.p2 = p2
        dist = MatrixNormalWishart(event_shape = (n,p1+p2), batch_shape = batch_shape + (dim,),pad_X=pad_X,X_mask=X_mask,mask=mask)
        super().__init__(dist,transition_mask=transition_mask)
    ...
    def update_obs_parms(self,XRY,lr,beta=None):  #only uses expectations
        Sigma = matrix_utils.block_diag_matrix_builder(XRY[0].ESigma(),torch.zeros(XRY[0].shape[:-2]+(self.p2,self.p2),requires_grad=False))
        mu = torch.cat((XRY[0].mean(),XRY[1]),dim=-2)
        prXR = MultivariateNormal_vector_format(mu=mu,Sigma=Sigma)
        self.obs_dist.update(prXR,Delta(XRY[2]),p=self.p,lr=lr,beta=beta)
