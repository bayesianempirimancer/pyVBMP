...
class BayesianFactorAnalysis():
    ...
    def __init__(self, obs_dim, latent_dim, batch_shape=(), pad_X=True):
        ...
        self.A = MatrixNormalGamma(event_shape = (obs_dim, latent_dim), batch_shape = batch_shape)
    ...
    def raw_update(self, Y, iters=1, lr=1.0, verbose=False):
        ...
        for i in range(iters):
            ...
            logZ = Res.sum(sample_dims)-pz.Res().sum(sample_dims)
            SEzz = pz.EXXT().sum(0)
            SEyy = (Y @ Y.transpose(-2,-1)).sum(0)
            SEyz = (Y @ pz.mean().transpose(-2, -1)).sum(0)
            N = torch.tensor(sample_shape).prod()
            # Update the parameters of the model
            if verbose is True:
                ...
                self.ELBO_last = logZ - self.KLqprior()
            self.A.ss_update(SEzz, SEyz, SEyy, N, lr=lr)
    ...
    # Compute the mean and covariance of the posterior distribution over Y
        ...
        Res = 0.5*self.A.ElogdetinvSigma() - 0.5*self.obs_dim*torch.tensor(2*torch.pi,requires_grad=False).log() + self.pz.Res()
    ...
    def postdict(self,Y):
        Y = Y.unsqueeze(-1)
        invSigma, invSigmamu, Res = self.A.Elog_like_X(Y)
        return MultivariateNormal_vector_format(invSigma=invSigma + torch.eye(self.latent_dim, requires_grad=False), invSigmamu=invSigmamu)  # sum over roles
