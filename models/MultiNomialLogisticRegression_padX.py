import torch
import numpy as np
from .dists import MVN_ard, MatrixNormalWishart
...
class MultiNomialLogisticRegression():
    ...
    def __init__(self,n,p,batch_shape = (),pad_X=True):
        ...
        self.beta.mu = torch.randn_like(self.beta.mu)/np.sqrt(self.p*self.n)
        self.pad_X = pad_X               
    ...
    def raw_update(self,X,Y,iters = 4, p=None,lr=1.0,beta=None,verbose=False):
        ...
        if p is None:
            ...
            SEyx = ((YmN*EX)*p.view(p.shape+(1,1,1))).sum(0)
        ...
        for i in range(iters):
            ...
            Ew = Ew.view(Ew.shape + (1,1))   # expands to sample x batch x n x 1 x 1
            if p is None:
                SExx =  (Ew*EXXT).sum(0)  # sample x batch x n x p x p 
            else:
                SExx = (Ew*EXXT*p.view(p.shape+(1,1,1))).sum(0)  # sample x batch x n x p x p

            while SExx.ndim > self.event_dim + self.batch_dim + 1:
                SExx = SExx.sum(0)
            
            if verbose is True: 
                ELBO = (SEyx*self.beta.mean()).sum((-3,-2,-1)) - (pgb*(0.5*pgc).cosh().log()).sum((0,-1)) - pgb.sum((0,-1))*np.log(2) - self.KLqprior().sum()
    ...
    def update(self,pX,pY,iters=1,p=None,lr=1,beta=None,verbose=False):  
        ...
        Warning('MNLR update does not use p ')
        ...
        for i in range(iters):
            ...
            if verbose is True: 
                ...
                self.ELBO_last = ELBO
                print("Percent Change in ELBO: ",((ELBO-ELBO_last)/ELBO_last.abs()*100).data)
            self.beta.ss_update(SExx,SEyx,lr=lr,beta=beta)
