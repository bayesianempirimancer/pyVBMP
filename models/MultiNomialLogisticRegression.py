...
class MultiNomialLogisticRegression():
    ...
    def __init__(self,n,p,batch_shape = (),pad_X=True):
        ...
        self.beta.mu = torch.randn_like(self.beta.mu)/np.sqrt(self.p)
        self.pad_X = pad_X               
    ...
    def raw_update(self,X,Y,iters = 4, p=None,lr=1.0,beta=None,verbose=False):
        ...
        for i in range(iters):
            ...
            if verbose is True: 
                ELBO = (SEyx*self.beta.mean()).sum((-3,-2,-1)) - (pgb*(0.5*pgc).cosh().log()).sum(sample_dims).sum(-1) - pgb.sum(sample_dims).sum(-1)*np.log(2) - self.KLqprior()
