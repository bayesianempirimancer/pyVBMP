...
class LinearDynamicalSystems():
    def __init__(self, obs_shape, hidden_dim, control_dim = 0, regression_dim = 0, obs_model = None, latent_noise = 'independent', batch_shape = (), A_mask =None, B_mask = None):
        ...
        if(latent_noise=='shared'):
            ...
            mu_0 = torch.zeros(batch_shape + offset + (hidden_dim, hidden_dim+control_dim),requires_grad=False) + torch.eye(hidden_dim,hidden_dim+control_dim,requires_grad=False), 
            mask=A_mask, 
            pad_X = False,
            ).to_event(len(obs_shape)-1)
        else:
        ...
        if(obs_model is None):
            self.obs_model = MatrixNormalWishart(
                mu_0 = torch.zeros(batch_shape + obs_shape + (hidden_dim + regression_dim,),requires_grad=False), 
#                U_0  = torch.zeros(batch_shape + obs_shape + obs_shape[-1:],requires_grad=False) + torch.eye(obs_shape[-1],requires_grad=False)*5.0,
                mask = B_mask,
                pad_X = False,
                ).to_event(len(obs_shape)-1)
