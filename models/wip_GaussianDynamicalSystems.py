...
class LinearDynamicalSystems():
    def __init__(self, obs_shape, hidden_dim, control_dim = 0, regression_dim = 0, obs_model = None, latent_noise = 'independent', batch_shape = (), A_mask =None, B_mask = None):
        ...
        print("LDS:  ELBO needs a fixin... include Res in forward pass")
        ...
        self.x0 = NormalInverseWishart(offset + (hidden_dim,),batch_shape)
        if(latent_noise=='shared'):
            self.A = MatrixNormalWishart(offset + (hidden_dim, hidden_dim+control_dim),batch_shape, pad_X = False, mask=A_mask)
        else:
            self.A = MatrixNormalGamma(offset + (hidden_dim,hidden_dim+control_dim), batch_shape, pad_X = False, mask=A_mask)

        if(obs_model is None):
            self.obs_model = MatrixNormalWishart(obs_shape + (hidden_dim + regression_dim,), batch_shape, mask = B_mask, pad_X = False)
    ...
    def update(self,y,u=None,r=None,p=None,iters=1,lr=1.0,verbose=False):
        L = -torch.tensor(torch.inf,requires_grad=False)
        L_last = L
...
#                self.T=self.T.unsqueeze(-1)
    ...
    def log_likelihood_function(self,Y,R):
        ...
        Residual = Residual.squeeze(-1).squeeze(-1) + 0.5*self.obs_model.ElogdetinvSigma()- 0.5*(self.obs_dim)*self.log2pi  # is log2pi superfluous?
        ...
        return invSigma_t_t, invSigmamu_t, Residual   # Residual does not include invSigma_t_t
    ...
    def backward_step_with_Residual(self, invGamma, invGammamu, Residual, invSigma_like, invSigmamu_like, Residual_like, U):
        ...
        Residual = Residual + Residual_like - 0.5*(U.transpose(-2,-1)@self.ATQA_u_u@U).squeeze(-1).squeeze(-1)
        Residual = Residual + 0.5*self.A.ElogdetinvSigma() - 0.5*(self.hidden_dim)*self.log2pi

        Residual = Residual + (0.5*invSigmamu_tp1.transpose(-2,-1) @ Sigma_tp1_tp1 @ invSigmamu_tp1).squeeze(-1).squeeze(-1)
        Residual = Residual + 0.5*Sigma_tp1_tp1.logdet() + 0.5*self.hidden_dim*self.log2pi

        mu = invGamma.inverse()@invGammamu
        post_Residual = - 0.5*(mu * invGammamu).squeeze(-1).sum(-1) + 0.5*invGamma.logdet() - 0.5*self.hidden_dim*self.log2pi
    ...
    def forward_backward_loop(self,y,u,r):
        ...
        Residual = - 0.5*self.x0.EXTinvUX() + 0.5*self.x0.ElogdetinvSigma() - 0.5*self.hidden_dim*self.log2pi
