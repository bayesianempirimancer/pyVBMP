...
from models.dists import MatrixNormalWishart, MatrixNormalGamma
from models.dists import MultiLinearNormalWishart


from models.dists import NormalInverseWishart
from models.HHMM import HHMM
obs_dist = NormalInverseWishart(mu_0 = torch.zeros(5,2,requires_grad=False))

dims = (3,)
model = HHMM(obs_dist = obs_dist, level_dims=dims)
...
p = sum(p_list)
noise_var = 0.1
#X = torch.randn(num_samples,p,1)
X = torch.randn(p,p)@torch.randn(num_samples,p,1)/np.sqrt(p)
X_list = [X[:,0:p_list[0],:]]
W_list = []
Y=0
start = p_list[0]
for i in range(len(p_list)-1):
    halt = start + p_list[i+1]
    X_list.append(X[:,start:halt,:])
    start = halt
for i in range(len(p_list)):
    W_list.append(torch.randn(batch_shape+(n,p_list[i]))/np.sqrt(p))
    Y = Y + W_list[i]@X_list[i]

Y = Y + torch.randn_like(Y)*np.sqrt(noise_var) 
Y = Y + 2*torch.randn(n,1)

model = MultiLinearNormalWishart(n,p_list,batch_shape=(),pad_X = pad_X, noise_type = 'Wishart')
m0 = MatrixNormalWishart(mu_0=torch.zeros(n,p),pad_X=pad_X)
# model = MultiLinearNormalWishart(n,p_list,batch_shape=(),pad_X = pad_X, noise_type = 'Gamma')
# m0 = MatrixNormalGamma(mu_0=torch.zeros(n,p),pad_X=True)

t = time.time()
model.raw_update(X_list,Y,iters=4,lr=1)
print("multilinear run time = ",time.time()-t)

t = time.time()
m0.raw_update(X,Y,lr=1)
print("NormalGamma Run time = ",time.time()-t)
pY0 = m0.predict(X)[0]
Yhat0 = pY0.mean()

pY = model.predict(X_list)[0]
Yhat = pY.mean()
MSE = (Y-Yhat).pow(2).mean()
MSE0 = (Y-Yhat0).pow(2).mean()
plt.scatter(Y,Yhat)
plt.scatter(Y,Yhat0)
plt.plot([Y.min(),Y.max()],[Y.min(),Y.max()])
plt.title('Prediction:  MSE/MSE0 = '+str(MSE.numpy()/MSE0.numpy()))
plt.show()
print('MSE = ',MSE)

W_hat = W_list[0]
A_hat = model.A[0].weights()
bias = 0.0
for i in range(1,len(p_list)):
    W_hat = torch.cat((W_hat,W_list[i]),dim=-1) 
    A_hat = torch.cat((A_hat,model.A[i].weights()),dim=-1)    
    bias = bias + model.A[i].bias()
plt.scatter(W_hat,A_hat)
plt.scatter(W_hat,m0.weights())
plt.plot([W_hat.min(),W_hat.max()],[W_hat.min(),W_hat.max()])
plt.title('Weights')
plt.show()
...
from models.dists import MatrixNormalGamma
n=10
p=20
n_samples = 4000
batch_num = 4
...
W0 = MatrixNormalGamma(torch.zeros(n,p),torch.ones(n),torch.eye(p),pad_X=pad_X)
W1 = MatrixNormalGamma(torch.zeros(n,p),torch.ones(n),torch.eye(p),X_mask=X_mask,pad_X=pad_X)
W2 = MatrixNormalGamma(torch.zeros(n,p),torch.ones(n),torch.eye(p),mask=X_mask.expand(n,p),pad_X=pad_X)
X=torch.randn(n_samples,p)
Y=torch.zeros(n_samples,n)
for i in range(n_samples):
    Y[i,:] = X[i:i+1,:]@w_true.transpose(-1,-2) + b_true.transpose(-2,-1)*pad_X + torch.randn(1)/100.0
...
model = dMixtureofLinearTransforms(n,p,nc+1,batch_shape=(),pad_X=True)
...
model = Mixture(dist)
model.update(data.unsqueeze(-3),iters=10,lr=1,verbose=True)
...
model = dMixtureofLinearTransforms(n,p,dim,pad_X=False,type = 'Gamma')
model.raw_update(X,Y,iters=20,lr=1,verbose=True)

model2 = MixtureofLinearTransforms(n,p,dim,pad_X=False,type = 'Gamma')
...
model2 = dMixtureofLinearTransforms(n,p,nc,batch_shape=batch_shape,pad_X=pad_X)
model3 = NLRegression_Multinomial(n,p,nc,batch_shape=batch_shape)
model4 = MixtureofLinearTransforms(n,p,nc,batch_shape=batch_shape,pad_X=pad_X)
