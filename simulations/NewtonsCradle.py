...
class NewtonsCradle():
    ...
    def initialize(self,init_type='random'):
        ...
        if(init_type=='1 ball object'):
            theta = 2*np.pi*(torch.rand(self.batch_size,1) - 0.5)/100 - np.pi/2*(torch.rand(self.batch_size,1)+2)/3 
        ...
        if(init_type=='2 ball object'):
            theta = 2*np.pi*(torch.rand(self.batch_size,2) - 0.5)/100 - np.pi/2*(torch.rand(self.batch_size,1)+2)/3
            theta = theta.sort(-1)[0]
            other_thetas = 2*np.pi*(torch.rand(self.batch_size,self.n_balls-2) - 0.5)
            other_thetas = other_thetas.sort(-1)[0]/100.0
            theta_0 = torch.cat((theta,other_thetas),-1)    
        if(init_type=='3 ball object'):
            theta = 2*np.pi*(torch.rand(self.batch_size,3) - 0.5)/100 - np.pi/2*(torch.rand(self.batch_size,1)+2)/3
            theta = theta.sort(-1)[0]
            other_thetas = 2*np.pi*(torch.rand(self.batch_size,self.n_balls-3) - 0.5)
            other_thetas = other_thetas.sort(-1)[0]/100.0
            theta_0 = torch.cat((theta,other_thetas),-1)    
        if(init_type=='4 ball object'):
            theta = 2*np.pi*(torch.rand(self.batch_size,4) - 0.5)/100 - np.pi/2*(torch.rand(self.batch_size,1)+2)/3
            theta = theta.sort(-1)[0]
        ...
        if(init_type == '1 + 1 ball object'):
            thetaL = 2*np.pi*(torch.rand(self.batch_size,1) - 0.5)/100 - np.pi/2*(torch.rand(self.batch_size,1)+2)/3 
            thetaR = 2*np.pi*(torch.rand(self.batch_size,1) - 0.5)/100 + np.pi/2*(torch.rand(self.batch_size,1)+2)/3 
            thetaL = thetaL.sort(-1)[0]
            thetaR = thetaR.sort(-1)[0]
            other_thetas = 2*np.pi*(torch.rand(self.batch_size,self.n_balls-2) - 0.5)
            other_thetas = other_thetas.sort(-1)[0]/1000.0
            theta_0 = torch.cat((thetaL,other_thetas,thetaR),-1)    

        if(init_type == '1 + 2 ball object'):
            thetaL = 2*np.pi*(torch.rand(self.batch_size,1) - 0.5)/100 - np.pi/2*(torch.rand(self.batch_size,1)+2)/3 
            thetaR = 2*np.pi*(torch.rand(self.batch_size,2) - 0.5)/100 + np.pi/2*(torch.rand(self.batch_size,1)+2)/3 
            thetaL = thetaL.sort(-1)[0]
            thetaR = thetaR.sort(-1)[0]
            other_thetas = 2*np.pi*(torch.rand(self.batch_size,self.n_balls-3) - 0.5)
            other_thetas = other_thetas.sort(-1)[0]/1000.0
        ...
        if(init_type == '1 + 3 ball object'):
            thetaL = 2*np.pi*(torch.rand(self.batch_size,1) - 0.5)/100 - np.pi/2*(torch.rand(self.batch_size,1)+2)/3 
            thetaR = 2*np.pi*(torch.rand(self.batch_size,3) - 0.5)/100 + np.pi/2*(torch.rand(self.batch_size,1)+2)/3 
            thetaL = thetaL.sort(-1)[0]
            thetaR = thetaR.sort(-1)[0]
            other_thetas = 2*np.pi*(torch.rand(self.batch_size,self.n_balls-4) - 0.5)
            other_thetas = other_thetas.sort(-1)[0]/1000.0
        ...
        if(init_type == '2 + 3 ball object'):
            thetaL = 2*np.pi*(torch.rand(self.batch_size,2) - 0.5)/100 - np.pi/2*(torch.rand(self.batch_size,1)+2)/3 
            thetaR = 2*np.pi*(torch.rand(self.batch_size,3) - 0.5)/100 + np.pi/2*(torch.rand(self.batch_size,1)+2)/3 
            thetaL = thetaL.sort(-1)[0]
            thetaR = thetaR.sort(-1)[0]
