...
from examples.animate_results import animate_results
...
data = (data/data.std()).clone().detach()

model = DMBD(obs_shape=data.shape[-2:],role_dims=(1,2,1),hidden_dims=(2,2,2),batch_shape=(),regression_dim = 0, control_dim=0,number_of_objects=1)
...
for i in range(iters):
    model.update(data,None,None,iters=4,latent_iters=1,lr=1,verbose=True)
