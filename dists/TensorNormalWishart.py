...
import torch
from .Wishart_UnitDet import Wishart_UnitDet as Wishart
from .Gamma import Gamma

class TensorNormalWishart():

    def __init__(self,event_shape,batch_shape=()):
        self.dims = event_shape
        self.mu_0 = torch.zeros(batch_shape + event_shape,requires_grad=False)
        self.mu = torch.randn(batch_shape + event_shape,requires_grad=False)/torch.sqrt(torch.prod(torch.tensor(self.dims,requires_grad=False)))
        ...
        for i in range(len(event_shape)):
            self.invU.append(Wishart(event_shape = 2*event_shape[i:i+1], batch_shape = batch_shape))
        self.alpha = Gamma(event_shape = (), batch_shape = batch_shape)
    ...
    def _raw_update(self,X,lr=1.0,beta=None):
        ...
        N = torch.prod(torch.tensor(sample_shape,requires_grad=False))*torch.ones(self.batch_shape)
        ...
        self.alpha.ss_update((torch.prod(torch.tensor(self.event_shape,requires_grad=False))*torch.prod(torch.tensor(sample_shape,requires_grad=False))/2.0).expand(self.batch_shape).float(), temp/2.0 ,lr=lr,beta=beta)
    ...
    def KLqprior(self):
        ...
        KL = 0.5*self.lambda_mu_0*KL + 0.5*(self.lambda_mu_0/self.lambda_mu - 1 + (self.lambda_mu/self.lambda_mu_0).log())*torch.prod(torch.tensor(self.dims,requires_grad=False))
    ...
    def Elog_like(self,X):
        ...
        ELL = ELL - 0.5*torch.prod(torch.tensor(self.dims,requires_grad=False))*torch.log(2*torch.tensor(torch.pi,requires_grad=False)) + 0.5*self.ElogdetinvSigma()
        return ELL
    ...
    def ElogdetinvSigma(self):
        res = torch.prod(torch.tensor(self.dims,requires_grad=False))*self.alpha.loggeomean()
        for invU in self.invU:
