...
class MultivariateNormal_vector_format():
    ...
    def ss_update(self, SExx, SEx,lr=1.0):
        self.invSigma = SExx*lr + self.invSigma*(1-lr)
    ...
    def EResidual(self):
        if self.Residual is None:
            self.Residual = - 0.5*(self.mean()*self.EinvSigmamu()).sum(-1).sum(-1) + 0.5*self.ElogdetinvSigma() - 0.5*self.dim*torch.log(2*torch.tensor(torch.pi,requires_grad=False))
    ...
    def Elog_like(self,X):
        ...
        out = out - 0.5*self.dim*torch.log(2*torch.tensor(torch.pi,requires_grad=False)) + 0.5*self.ElogdetinvSigma()
